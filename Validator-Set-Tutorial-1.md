...
Create `Validator-set-tuto.json` file containing:
...
Each node corresponds to an account. The authorities (`Node0` at the beginning, then also `Node1`) seal blocks and sign them using their private key. `Alice` will send transactions and signs them as well. We will create an account for each of them. 

All nodes will run on 1 single machine. We will need to handle the networking ports carefully to avoid conflicts.
...
force_sealing = true
```
We are missing here also the address and password file for `Node0`'s account as well as the bootnodes' addresses corresponding to all the nodes that we will use in this tutorial. These fields are commented (notice the `#`) for the moment.
...
We will use this address and password file right away in our `node0.toml` configuration file. Add and uncomment the address as `engine_signer` in the `[mining]` section and add an `[account]` as follow. 
...
As explained previsously, we wish to have `Node0` as the unique validator for the beginning of the blockchain. 
Add `Node0`'s address to the `Validator-set-tuto.json` genesis file in the validators' list so that it looks as follow:
...
This means that a fixed list of validators will be used from block 0. `Node0`'s address is the only one authorized to seal blocks.
...
```
This assess that blocks are generated.

You should also copy in a text file the Pubic node address as we will use is later on.
```bash
...
We will use this address and password file right away in our `node1.toml` configuration file. Add and uncomment the address as `engine_signer` in the `[mining]` section and add an `[account]` as follow. 
...
`Alice`'s node configuration will be slighty different than the two validator's nodes that we have created previously.
It will not need any `[mining]` section. We will use the standard ports for HTTP and WebSockets RPCs so that `Alice`'s node can be easiely connected to the tools used later such as Remix. 
...
Run the following command to create a file named `alice.pwd` containing the password `alicepwd`:
...
password = ["alice.pwd"]
```
To make sure the file is valid, launch `Alice` using the command `parity --config ./alice.toml` and verify that you do not get any error in the console. 
Also copy in a text file the Pubic node address as we will use is later on.
```bash
...
Add the list of nodes to `Node0.toml`, `Ǹode1.toml` and `Alice.toml` so that each file contains lines similar to:
```toml
bootnodes = [
